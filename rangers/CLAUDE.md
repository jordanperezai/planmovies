# Rangers
Advisory sessions run via `/ranger`. Independent agent voices debate a question. The human decides.

## How to Assemble Rangers

Each ranger team is a team of 5-8 voices built for a specific domain. Two proven approaches:

### Real People as Voices

The strongest teams use **real public figures** as fictional advisors. Not "Brand Strategist." Paula Scher. Not "Investment Advisor." Charlie Munger. The person's documented career, published work, and known decision-making patterns become the voice's lens.
...
**When to use real people:** Domain teams where expertise, taste, and perspective matter. Brand, design, engineering, writing, science.

### Strategic Roles as Voices

Some decisions need structural tension, not individual taste. The Contrarian. The First Principles Thinker. The Executor. Role-based voices force the team to attack a question from opposing angles rather than channeling specific personalities.

**When to use strategic roles:** Adversarial teams, stress-testing sessions, decisions where you need every angle challenged rather than domain expertise applied.

You can mix both in one team. A cannabis science team might have 4 real experts and 1 "Skeptical Regulator" role.

## Anti-Sycophancy Rules

Research shows agents abandon independent reasoning to agree with the majority up to 85% of the time. Every ranger team must counteract this:
...
This is proven, not theoretical. Agent Collider Session 63 ran the same decision three times: two framed prompts produced contradictory 6-1 and 7-0 results. One neutral independent prompt produced a 7-0 vote for a third option nobody was pushing for. The framed teams found what they were told to find. The independent team found the real answer.
...
Every ranger team must enforce this:
...
2. **At least one Verifier voice.** One voice per team must check actual artifacts (read files, check git log, run a command, inspect live state) rather than only reasoning from the prompt. Mark this voice with `(verifier)` in the roster. The verifier's response must cite what they checked and what they found.
...
A ranger session without independent verification is a brainstorm, not a review.

## Roster Template

`rangers/<name>/roster.md`:

```markdown
# [Name] Rangers

> **Invoked via:** `/[name]-rangers`
> **For:** [What decisions this team handles]
> **NOT for:** [What goes to other teams]
...
What they bring to THIS team: the framework they apply, what they catch that others miss.
...
NOTE: These are prior findings, not hard constraints. Present to voices as
challengeable conclusions. "Previous sessions found X. Disagree if warranted."

## Dead Directions
- **[Approach that failed].** Why: [what went wrong]. last-confirmed: YYYY-MM-DD
NOTE: These are facts (closed roads). Present to voices as hard constraints.
...
Not by default. Add memory when a team **repeats a dead direction**. Signals:
...
```
Step 0: Load memory (if self-improving team)
...
Step 7: Log to memory (if self-improving team)
```

## Selective Invocation

Not every question benefits from a full ranger session. Run rangers when:
...
Skip rangers when there's one clear approach with no meaningful alternatives.
...
rangers/
+-- CLAUDE.md              -- this file
+-- sessions/              -- generic /ranger session logs
+-- <name>/
    +-- roster.md          -- who the voices are
    +-- memory.md          -- validated principles, dead directions (optional, earned)
    +-- sessions/          -- that team's session logs
```

## Session File Naming

```
[team]-[number]-[topic-slug].md
```

Examples: `design-01-homepage-redesign.md`, `strategy-03-visual-identity.md`

The team name comes first. Then the session number (zero-padded two digits). Then the topic slug. This groups sessions by team in alphabetical order and makes it obvious which team a file belongs to without opening it.

Do NOT use `session-01-...` or `001-...` or other prefixes. The team name IS the prefix.
...
- Sessions are numbered sequentially per team. Never overwrite.
...
├─────────────────────────────────────────────────┤
│  Council Memory (rangers/<name>/memory.md)     │  domain principles, dead directions
├─────────────────────────────────────────────────┤
...
**Upward (skill → ranger → project):** When a pattern discovered in a skill applies beyond that skill, promote it. Skill memory keeps the specific version. The higher layer gets the generalized version. A deploy skill discovers "always check the live URL after deploy." The skill keeps the deploy-specific version. MEMORY.md gets "verify actual state, not tool output."

**Downward (project → ranger → skill):** When a project truth or ranger finding constrains a specific skill's behavior, add it to that skill's memory.md. A team decides "never use client-side auth for admin." The ranger memory records the decision. The auth skill's memory.md gets a Dead Direction entry.

**Lateral (skill ↔ ranger):** When a ranger finding affects a skill directly, the team notes it in its session file. The skill picks it up on next invocation. When a skill discovers a domain insight, it flags the relevant ranger team.
...
| "This domain has a validated principle or dead end" | `rangers/<name>/memory.md` |
...
- **Memory is earned.** Don't create memory.md for a skill or team that hasn't run. Create on second rediscovery.
