...
- Logo LOCKED (now LIVE, 2026-05-30): 5 chunky rounded-tab row mark, Ember `#e53908` (lab=wrong; traced from pixels), lowercase `planmovies` Poppins 800, slogan "Get the row together." Nav = wordmark only (mark removed from nav per Jordan). Fill: open seats = ember OUTLINE (not dim fill — dim fill was 2.19:1, failed); filled = solid ember; `renderHeroRow()` live. Critique 30/40 baseline saved.
