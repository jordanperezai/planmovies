...
### Skill Memory vs Ranger Memory vs Project Memory
...
| **Ranger** | `rangers/<name>/memory.md` | Domain principles, dead directions, voice calibration | 20 entries per ranger |
| **Skill** | `.claude/skills/<name>/memory.md` | Process patterns specific to that skill | 20 entries per skill |

**Flow direction:**
- **Up** (skill to ranger to project): When a skill discovers something that applies beyond that skill, promote it. The skill keeps the specific version. The higher layer gets the generalized version.
- **Down** (project to ranger to skill): When a project truth constrains a specific skill, add it to that skill's memory.
- **Lateral** (skill to ranger): When a ranger finding affects a skill, the ranger notes it. The skill picks it up on next run.
...
| Skill memory hits 20 entries | Evict oldest or promote to ranger/project memory |
