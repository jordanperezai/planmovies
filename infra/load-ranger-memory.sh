#!/bin/bash
# Summarize ranger memory state at session start.
# Silent when all rangers are empty — only speaks when there's something to see.

cd "$(dirname "$0")/.." || exit 0

found=0

for mem in rangers/*/memory.md; do
  [ -f "$mem" ] || continue
  ranger=$(basename "$(dirname "$mem")")
  ...
  if [ $found -eq 0 ]; then
    echo "[ranger memory]" >&2
    found=1
  fi

  echo "  $ranger: ${validated} validated, ${dead} dead directions${latest:+ (latest: $latest)}" >&2
done
